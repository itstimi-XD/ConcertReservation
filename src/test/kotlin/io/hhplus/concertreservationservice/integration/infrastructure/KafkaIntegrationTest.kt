package io.hhplus.concertreservationservice.integration.infrastructure
...
class KafkaIntegrationTest {
    ...
    @Test
    fun `pretty topic에 인후를 발행하고 인후를 받아온다^_^`() {
        KafkaProducer<String, String>(producerProps).use { producer ->
            producer.send(ProducerRecord(topic, "key", "inhoo")).get()
        }
        ...
        KafkaConsumer<String, String>(consumerProps).use { consumer ->
            ...
            Assertions.assertEquals("inhoo", records.first().value())
        }
    }
}